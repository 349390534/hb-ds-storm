 ...
 *
 */
public class H5KafkaProducerTest {
	private static Logger logger = LoggerFactory.getLogger(H5KafkaProducerTest.class);
			...
			new Thread("Tpv" + (i + 1)){
				...
				public void run() {
					...
					while(true){
						...
						System.out.println(Thread.currentThread().getName() + " -pv:" + ++count+",mess:"+message);
						try {
			...
			new Thread("Tevent" + (i + 1)){
				...
				public void run() {
					...
					while(true){
						...
						System.out.println(Thread.currentThread().getName() + " -event:" + ++count+",mess:"+message);
						try {
							Thread.sleep(new Random().nextInt(200));
						} catch (InterruptedException e) {
							logger.error(key+"休眠异常",e);
